...
def process_file(filename, fields):
    ...
    with open(filename, "r") as f:
        ...
        for line in reader:
            #eliminamos (spider)
            line['rdf-schema#label'] = re.sub('\(.+\)','', line['rdf-schema#label'])
            #quitamos espacios
            line['rdf-schema#label'] = line['rdf-schema#label'].strip()
            ...
            if line['synonym'] == 'NULL':
                ...
                line['synonym'] = parse_array(line['synonym'].replace('*', "").strip())
            ...
            for c in fields:
                ...
                if re.search(r'_label', c):
                    temp['classification'][fields[c]] = line[c]
                else:
                    temp[fields[c]] = line[c]
