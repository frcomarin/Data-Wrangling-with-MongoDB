...
def update_name(name, mapping):
    ...
    if street_type:
        if street_type.group() not in expected:
            #http://www.dotnetperls.com/re-sub
            name = re.sub(street_type_re,mapping[street_type.group()], name)
            ...
            if not problemchars.search(tag.attrib['k']):
                if lower_colon.search(tag.attrib['k']):
                    if 'addr' in tag.attrib['k']:
                        ...
                        if 'street' in sub_attr[1]:
                            # It dosent work if I Clean de data
                            # node['address'][sub_attr[1]] = update_name(tag.attrib['v'],mapping)
